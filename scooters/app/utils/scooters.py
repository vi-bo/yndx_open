...
async def _enrich_address(
    scooters: tp.List[dto.Scooter], client: geocode.GeocoderClient
        ...
        futures = [
            client.get_address(scooter.location)
...
async def get_scooters(context: AppContext, params: GetScootersParams) -> tp.List[dto.Scooter]:
    scooters = [dto.Scooter.from_model(scooter) for scooter in await storage.get_scooters(context)]
